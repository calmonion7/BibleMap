...
function SidePanel({ nodeId, onSelectNode = () => {}, onBack = () => {}, canGoBack = false, onNodeLoaded, verseLang, setVerseLang, explorePersonId = null, onExplorePerson = () => {}, curatedIds = null, keyPeopleCards = null, onExploreJourney = () => {}, onOpenFamily = () => {}, onOpenPlace, onClose, stickyTop = 0 }) {
  ...
  function renderVerseLayer() {
    ...
    return (
      <VerseLayer
        title={placeVerseView.label}
        onClose={closePlaceVerseView}
        verseLang={verseLang}
      ...
      >
        <VerseBookTabs
          ...
          onSelect={(bookId) => setPlaceVerseView(prev => prev ? { ...prev, bookId } : prev)}
        />
        {overlay === null ? (
  ...
  function renderPersonVerseLayer() {
    ...
    return (
      <VerseLayer
        title={p.name}
        refLine={p.role}
        onClose={() => setPersonVerseView(null)}
        verseLang={verseLang}
        setVerseLang={setVerseLang}
      >
    ...
    <div style={{ fontFamily: 'var(--sans)' }}>
      ...
      {node.label === 'Book' && (
        <div style={{ padding: '12px 16px 20px', fontSize: 14 }}>
          {/* 단어 분포 진입은 책 상세 내비 바의 하위 메뉴 탭(App.renderBookNav)으로 이동 — 패널 내 버튼 없음 */}
          {/* 메타 칩 */}
                  ...
                  {mainPersons.map(p => {
                    ...
                    return p.journeyId ? (
                      ...
                      <div key={p.key} style={rowStyle}>
                        <span style={{ flex: 1, fontSize: 13, color: 'var(--ink-dim)' }}>{p.name}</span>
                      </div>
                    )
                  })}
                </div>
