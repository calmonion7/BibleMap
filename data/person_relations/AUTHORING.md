...
1. **저작 vs 프리베이크 분리** — LLM은 `verse`(앵커 참조)·`context`(문맥 범위)만 쓴다. 본문(`verseTextKo/En`·`contextKo/En`)은 **손으로 쓰지 않고** `generate_verse_text.py`가 getbible(한국어 개역 + KJV)에서 받는다(ADR-0003). 참조 형식 = 개역 약어 + "장:절"(예 `삼상 16:13`), 범위는 "장:절-절"(**같은 장 안에서만**). **`context`는 원칙 부여** — 앵커 절 하나가 국면 전체(주체·행위·결과)를 자체 완결로 서술하는 예외적인 경우(장 첫 절이라 앞으로 확장 불가한 도입절, 족보 정형구 등)에만 생략한다. 범위는 앵커 절 번호를 반드시 포함하고 보통 2~6절 창(앞의 원인·뒤의 결과 중 이해에 필요한 쪽).
...
7. **밀도 비례 · 공백 강요 금지**(CONTEXT 97 「공백이 곧 정확」) — 서사가 뒷받침하고, **valence를 가지며 시간에 따라 변하거나 의미 있는** 관계만 저작한다. person_events 분량이 큰 인물(예수·바울·모세·아브라함·다윗)은 깊게, 얕은 인물(셋·아벨·에녹)은 1~3개 또는 **생략**한다. 억지 관계를 만들지 않는다. 장소·집단(니느웨·블레셋 등)은 인물 관계가 아니다.
   - **집단의 경계** — "집단"은 지명·민족·직능 집합(블레셋·바리새인·길르앗 장로회)을 말한다. **이름이 있는 개인들의 묶음은 집단이 아니다** — 세바·살문나(기드온의 상대 두 왕)·사드락/메삭/아벳느고·셈/함/야벳·요셉 서사의 "형제들"은 하나의 relObj로 저작한다(nameKo에 묶음 표기). 집단으로 보이는 상대가 나오면 **named 개인으로 치환할 수 있는지 먼저 확인**하고(예: "미디안 왕" → 세바·살문나), 치환이 안 되면 생략한다. 파트 4·5·6에서 세 번 재도출된 판정.
...
- **재도출된 선례** (파트 2~6에서 계획 힌트와 어긋나게 반복 판정된 것 — 다음 저작은 여기서 시작한다):
  - **선지자가 왕을 책망하는 관계 = `선지자`**(대적 아님). 적대성은 국면 valence로. 예: 사무엘↔사울, 엘리야↔아합/여호람.
  - **상대가 왕/통치자면 `군주`**. 예: 요셉↔바로, 다니엘↔왕들, 느헤미야↔아닥사스다, 요나↔니느웨 왕, 빌라도(압력에 굴복한 재판관). 단 **배우자인 왕은 `연인`**(에스더↔아하수에로 — 배우자=연인 통일).
  - **이방인·외부 조력자 = `친구`**(`신하` 아님). 종속이 아니라 동역·동맹이기 때문. 예: 오바댜, 라합, 야엘, 아나니아.
  - **압제자와의 순수 대결 = `대적`**. 예: 모세↔바로(언약 왕-선지자 구도가 아님). `선지자`와의 구분선이 여기다.
  - **동명이인은 nameKo로 명시 구분** — 아비멜렉(그랄 왕) vs 아비멜렉(기드온의 아들), 요셉(족장) vs 요셉(예수의 양부, slug 없음). slug를 잘못 주면 오연결된다.
