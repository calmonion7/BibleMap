...
2. **런타임 JSON 오버레이** — `data/` 아래 JSON 파일. 백엔드가 메모리에 1회 로드(`lru_cache`)해 Neo4j 결과에 머지. 그래프에 없는 근거 구절·책↔사건 매핑을 덧붙인다.
...
프론트는 `apiGet`(단일 fetch 헬퍼, `frontend/src/api.js`) 하나로만 백엔드를 호출한다. 베이스 URL `API_BASE`는 `import.meta.env.VITE_API_URL`(프로덕션 `/api`, 개발 기본 `http://localhost:8000`)로 결정되며 모듈 내부 `const`다(외부로 export하지 않음). `apiGet`은 비-OK 응답이면 status를 담은 Error로 reject, 취소는 `AbortError`로 그대로 전파해 호출부가 `e.name === 'AbortError'`로 구분한다.

## 백엔드 (`backend/app`)

- **진입점 `backend/app/main.py`** — `FastAPI(lifespan=...)`. lifespan에서 5개 라벨(`Person/Place/Event/PeopleGroup/Book`)에 `theographic_id` 인덱스를 멱등 생성(실패해도 계속). CORS는 GET만 허용(`allow_origins=["*"]`). 라우터 4개(`nodes/events/search/books`)를 include.
- **`backend/app/db.py`** — Neo4j 드라이버 싱글턴(`get_driver`, 전역 lazy). URI/USER/PASSWORD는 환경변수, 비밀번호 없으면 RuntimeError.
- **`backend/app/overlays.py`** — `data/` JSON 로더. `_resolve`가 `DATA_DIR`(기본 `/app/data`) → 레포 `data/` 순으로 파일을 찾는다. `@lru_cache(maxsize=1)`로 노출하는 함수는 **2개뿐**: `book_events_raw()`(`{bookId:[eventId]}`), `event_verses()`(사건별 근거 구절). (task-85에서 고아 헬퍼 `approx_years()`가 제거됐다.)
- **라우트 (`backend/app/routes/`)**
  ...
  - `events.py` — `/events`(타임라인 전체 사건; Neo4j 사건 + `_load_approx_book_index()`로 만든 역방향 `{eventId:[book]}` 추정책 머지, 둘 다 `lru_cache`. 이 인덱스가 `overlays.book_events_raw()`의 **유일한 런타임 소비처**다 — 제거된 `/books`가 아니다), `/event/{id}/verses`(오버레이 그대로). 응답에 `Cache-Control: max-age=300`.
  - `search.py` — `/search?q=`(nameKo/name CONTAINS, 정확>접두>부분 rank 정렬, `SEARCH_LIMIT`=20 상한).
  - `books.py` — `/books-overview` **단 하나의 엔드포인트**(개요용 전체 책, `bookOrder ASC`, no-store). task-85에서 죽은 `GET /books`가 제거됐다.
...
런타임 경로 밖. 일회성/배치 실행으로 그래프를 채우거나 `data/` 오버레이를 생성한다. 모두 `NEO4J_*` 환경변수로 직접 접속, MERGE 기반 멱등.
...
- **상태 훅** — `useNodeSelection.js`(선택 노드 + 히스토리 스택 + Person 이벤트 ID 집합 + `selectedNodeMeta`; `selectNode`는 `useCallback([])`로 참조 안정화 — selectedNode 변경이 MapView effect를 재실행해 fetch가 abort되는 버그 방지, 최신값은 내부 `selectedNodeRef`로 읽음. `selectedNodeRef`는 훅 반환 객체에 노출하지 않는 내부 ref다(task-86). `handleNodeLoaded` 콜백으로 SidePanel이 받은 노드 메타를 역수신해 중복 fetch 회피), `useSearch.js`(250ms 디바운스 검색 + AbortController 경쟁 차단 + 타입 필터/키보드 하이라이트).
- **3 뷰**
  - `MapView.jsx` — MapLibre GL 지도(아래 별도 절).
  - `TimelineView.jsx` — `/events` 기반 연대 타임라인. startDate별 그룹핑, 사건 근거 구절 인라인 드릴다운(`/event/{id}/verses`, out-of-order 응답을 `openEventRef`로 방어), bookFilter/personFilter 지원. 본문(textKo/textEn)은 오버레이에 미리저장돼 추가 fetch 없음(ADR-0003).
  - `BibleOverviewView.jsx` — `/books-overview` 기반 장르별 책 카드 그리드(구약 5 / 신약 5 장르 순서·한글 메타 하드코딩).
- **`SidePanel.jsx`** — 선택 노드 상세. `/node/{id}` 호출 → 노드 속성 + 타입별 그룹 이웃(`REL_KO` 관계 한글화) + 절 본문. Person/Book/Place 전용 블록(성품·시대배경·대표구절). `onNodeLoaded` 콜백으로 메타를 App에 역전달.
...
- **`mapLayers.js` — 정적 지도 설정 + 이벤트 등록 + 팝업**.
  ...
  - `setupMapSources(map)` — 소스/레이어 일괄 등록: `hull-source`(Person 볼록껍질, 마커 아래 먼저), `places-source`(클러스터 on, `clusterMaxZoom:13`/`clusterRadius:18`/`clusterMinPoints:4` — `frontend/src/mapLayers.js:143-146`; `clusterRadius`는 마커 원이 실제 겹칠 때만 묶이도록 18(task-76 복원, 12~14 미만 금지), `clusterMinPoints:4`는 동일/근접 좌표 2~3개는 버블 대신 라벨 표시·4개+만 클러스터(task-84) — circle/label/cluster/cluster-count 레이어), `place-spider-source`(겹침 분산), `event-ring-source`(사건 링). 각 마커는 shadow+circle+label 3중 레이어. 베이스맵은 Esri NatGeo 래스터 타일, glyph는 protomaps 폰트(둘 다 외부 URL).
  - `registerEventHandlers(map, {collapseRing, collapseSpider, expandPlace, spiderifyPlaces, onSelectNode, popupRef, expandedPlaceRef})` — 클릭/호버 핸들러 일괄 바인딩: 마커 클릭 시 겹침이면 spiderify·아니면 ring expand(같은 장소 재클릭은 collapse), 클러스터 클릭은 `getClusterExpansionZoom`으로 `easeTo` 확대, 스파이더 마커 클릭은 원위치로 펼침, 빈 곳 클릭은 전체 collapse + 팝업 제거. `onSelectNode`로 App에 선택 전파.
  - `placePopupHTML` / `escapeHtml`(모듈 내부) — 마커 팝업 HTML 생성·이스케이프(XSS 방어, task-78).
- **`mapRingController.js` — 링/스파이더 애니메이션 컨트롤러 팩토리**.
  ...
  - 반환: `collapseRing` / `collapseSpider` / `expandPlace` / `spiderifyPlaces` / `destroy`. `expandPlace`는 `/node/{id}/neighbors/grouped`를 fetch해 Event 이웃을 zoom-adaptive 반경(화면 80px→degrees)으로 `requestAnimationFrame` 링 애니메이트(이전 fetch는 AbortController로 취소, 실패 시 `setError`로 피드백). `spiderifyPlaces`는 겹친 마커를 같은 방식으로 분산. `destroy()`가 진행 프레임·진행 fetch를 정리.

`MapView`의 초기화 effect는 `createRingController`로 컨트롤러를 만들고 `expandPlaceRef.current`에 `expandPlace`를 연결한 뒤, `map.on('load')`에서 `setupMapSources`+`registerEventHandlers`를 호출한다. 선택반응 effect는 `/node/{id}/places`를 받아 `placesToGeoJSON`으로 마커를 그리고, Person+3장소↑면 `convexHull`로 hull을 그리며, `isPrimary` 장소가 있으면 카메라 정착(moveend / 700ms 폴백 타이머) 후 링을 자동 펼친다(`isPrimary` 없는 인물/집단은 `coreBounds`로 전체 프레이밍). 빈 `/places`면 "위치 정보 없음" 안내를 띄운다. 언마운트 시 `ring.destroy()` + 팝업·ref 정리 + `map.remove()`.

## 빌드 / 배포

- **프론트** — Vite(`vite.config.js`). `manualChunks`로 `maplibre-gl`은 `maplibre` 청크, 나머지 node_modules는 `vendor` 청크로 분리. `npm run build` → `frontend/dist`. React 19 / maplibre-gl 5 / lucide-react / vite 8 / eslint 10.
- **nginx** — `nginx/nginx.conf`. `/api/` → `api:8000` 프록시, 정적 자산 1년 immutable 캐시, `index.html` no-cache, SPA fallback(`try_files $uri /index.html`).
- **배포** — `deploy.sh`(프론트 빌드 → API 이미지 빌드 → 컨테이너 재시작 → `inject_ko_names.py` 한글명 주입, lock 파일로 중복 방지), `.github/workflows/deploy.yml`(`self-hosted` 러너, main push 시 `git reset --hard origin/main` 후 `deploy.sh`). 백엔드 `backend/Dockerfile`(python:3.12-slim + uvicorn). 백엔드 의존성: `fastapi==0.136.3` / `neo4j==6.2.0` / `uvicorn==0.49.0`.
