...
│   ├── overlays.py          # 런타임 오버레이 로더 (_resolve/_resolve_dir/_load + 캐시 로더 6종 + curated_person_id)
...
└── scripts/                 # 빌드타임 배치 (API 밖에서 실행)
    ...
    ├── validate_*.py        # 데이터 규칙 기계 검증 (validate_god_reliance.py, validate_person_context.py 포함)
...
└── src/
    ...
    ├── App.jsx              # SpineHeader + 스테이지 상태 머신 + 레이아웃 셸 (7 스테이지 렌더 분기 + 스테이지별 내비 바 + 무좌표 여정 분기)
    ...
    ├── useStageNavigation.js# 스테이지/URL/히스토리 상태 머신 훅 (7 스테이지, explorePersonSlug·getPersonSlug 포함)
...
- **컬렉션형 오버레이**: `<entity_or_topic>/<collection>.json` 꼴. 컬렉션 파일명은 담는 엔티티 복수형(`events.json`, `people.json`, `places.json`, `books.json`, `groups.json`, `relations.json`, `verses.json`, `identity.json`, `dedupe.json`, `genealogy.json`, `mothers.json`, `index.json`, `seal_slugs.json`)이다. 예: `book_events/books.json`, `event_verses/events.json`, `bible/verses.json`, `person_slugs/seal_slugs.json`. 예외: `data/word_distribution.json`·`data/word_sentiment.json`은 서브디렉터리 없이 최상위에 놓인 단일 파일이다.
...
- `validate_*` — 데이터 규칙을 기계 검증. 예: `validate_event_chronology.py`, `validate_traits.py`, `validate_person_context.py`(ADR-0027 2단 계층 반영 — 최소 86명·role ≤80자·intro는 있을 때만 300자 검사), `validate_god_reliance.py`.
...
- 컴포넌트: PascalCase `.jsx`(`SidePanel.jsx`, `FamilyTree.jsx`, `PersonIntro.jsx`, `SpineHeader.jsx`, `VerseLayer.jsx`, `PersonMiniCard.jsx`). 뷰 컴포넌트는 접미사 `View`(`MapView`, `TimelineView`, `RelationsView`, `BibleOverviewView`, `WordDistributionView`, `RelianceView`); `PersonHub`·`PersonIntro`·`FamilyTree`·`TourList`·`JourneyList`·`SidePanel`·`SpineHeader`·`VerseLayer`·`PersonMiniCard`는 예외 없이 전체화면/패널/쉘 단위 컴포넌트다. `personSymbols.jsx`는 컴포넌트(`PersonSymbol`)와 심볼 데이터(`SYMBOLS`)를 함께 담는 유일한 카멜케이스 `.jsx`다.
