...
- 오버레이 로더: `backend/app/overlays.py` — `data/` 하위 JSON을 `functools.lru_cache(maxsize=1)`로 1회 로드. `_resolve`/`_resolve_dir`가 `DATA_DIR`(기본 `/app/data`) → 레포 `data/`(`_REPO_DATA_DIR`) 순으로 해석하고, 없으면 `[Overlays]` 경고 후 빈 데이터 폴백. 로더 14종: `book_events_raw`, `event_verses`, `bible_verses`, `word_distribution`, `books_ko`, `chapter_summaries`, `chapter_sections`, `quotations`, `messianic_prophecies`, `covenants`, `parables_miracles`, `place_coords`, `topical_verses`, `verse_persons`. 그 외 헬퍼 `curated_person_id(events)` 하나(큐레이션 신원 규약 단일 지점).
- 라우트 모듈: `backend/app/routes/*.py` 14개(`__init__.py` 포함). `@router.get` 기준 **30개 GET 엔드포인트**(POST/PUT/DELETE 없음). 응답은 대부분 `JSONResponse` + `Cache-Control` 헤더(`max-age=300` 또는 `public, max-age=3600`, 예외로 `books.py`의 `/books-overview`는 `no-store`). 집계형 계산은 `lru_cache`(대부분 `maxsize=1`, 일부 `maxsize=66/256/2048/None`)로 프로세스 내 캐시 → **데이터 변경 시 API 재시작 필요**.
- 데이터/스크립트: `backend/scripts/*.py` 44개. 접두사별 — `load_*` 8(Neo4j 적재), `generate_*` 11(오버레이 생성), `inject_*` 6(Neo4j 속성 주입), `validate_*` 13(검증), `build_*` 3(색인 빌드), `apply_*` 1(`apply_event_dedupe.py`). 앱 코드는 이 스크립트를 import하지 않는다(빌드타임/운영 전용 경계).
...
- npm 스크립트(`frontend/package.json`): `dev`(vite), `build`(vite build), `lint`(eslint .), `preview`(vite preview). 패키지 매니저는 npm(`frontend/package-lock.json` 커밋됨, lockfileVersion 기반 재현). yarn/pnpm 설정 파일 없음.
...
- `docker-compose.yml` — 3 서비스:
  ...
  - `nginx`: 이미지 `nginx:alpine`. 포트 `8080:80`. 볼륨 `./frontend/dist:/usr/share/nginx/html:ro`, `./nginx/nginx.conf:/etc/nginx/nginx.conf:ro`. `depends_on: api`.
...
- `deploy.sh`(레포 루트, 실행 권한 있음):
  ...
  4. **검증 게이트**: `bash scripts/check.sh` — `PIPESTATUS[0]` 비0이면 배포 중단(`exit 1`).
  5. `[1/3]` 프론트 빌드: `cd frontend && npm install --silent && npm run build --silent`.
  6. `[2/3]` `docker compose -p biblemap build api`.
  7. `[3/4]` `docker compose -p biblemap up -d api nginx`.
  8. `[4/4]` `python3 backend/scripts/inject_ko_names.py`를 최대 15회 재시도(2초 간격, Neo4j 준비 대기). 15회 실패 시 배포 중단.
  - 주의: `deploy.sh`는 `load_*` 적재 스크립트를 실행하지 않는다 — Neo4j 그래프 적재는 별도 수동 실행.
- 배포 전 검증 게이트 `scripts/check.sh`(AI 불요, 단독 실행 가능, `set -u`):
  - 파일 기반 데이터 검증 12종을 `python3 -m backend.scripts.validate_<name>`으로 실행: `covenants`, `messianic_prophecies`, `parables_miracles`, `topical_verses`, `pm_map_coverage`, `chapter_sections`, `chapter_summaries`, `quotations`, `person_context`, `god_reliance`, `traits`, `era_bands_consistency`.
  - ESLint: `frontend/node_modules` 있으면 `npx --no-install eslint src`, 없으면 스킵-경고.
