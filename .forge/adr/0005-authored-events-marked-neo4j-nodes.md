...
- `authored_events`(서신서·계시록 정황 앵커: 바울 투옥, 요한 밧모 유배 등)는 theographic `verses`가 없어 `/event/{id}/verses`가 빈 배열 → 📖 근거 구절 드릴다운이 없다(의도된 것 — ⚡ 정황 앵커이지 📖 근거 사건이 아님).
- **인물 여정 저작 이벤트(`person_events`: 아브라함·다윗·예수 등)**는 창세기·복음서·사도행전 같은 내러티브 책이 실제로 기록한 사건이다. 이 ADR의 CONTAINS_BOOK 제외 대상이 아님 — `generate_person_event_verses.py`로 구절 데이터를 생성하고 `load_person_events.py`가 CONTAINS_BOOK 관계를 생성하며, 📖 근거 칩과 드릴다운을 가진다(task-66).
