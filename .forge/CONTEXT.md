...
각 인물이 "얼마나 하나님을 의지했나"를 나타내는 **퍼센트 지표**. 그 인물의 [[하나님 상호작용 mode (God-interaction mode)]] 순간들을 집계해 **(물음-응답 + 물음-침묵 + 순종한 부르심) ÷ (전체 순간) × 100** 으로 계산한다("하나님과 정렬" 정의, ADR-0023). 인물↔인물 상호 관계가 아니라 **인물↔하나님** 한 축의 궤적이며, **구절 근거가 있는 주요 순간들에 대한 큐레이션 해석** 이지 전수 통계가 아니다 — 기록이 희소한 인물(아벨·에녹 등)은 "표본 적음" 뱃지로 신뢰도 한계를 밝힌다. 헤드라인 %(도넛 게이지)·mode 분해 막대·생애 궤적(연도축 mode색 점)·은혜 하이라이트(`독단-개입`)·인물 간 랭킹·성경 평균 대비 백분위로 [[화면-단계-stage]] 탐험 뷰의 전용 내비 탭에 렌더한다. → [[하나님 상호작용 mode (God-interaction mode)]], [[인물-관계-person-relations]]
