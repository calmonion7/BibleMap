...
- **관계 유형 (type)**: valence와 **직교하는 구조적 분류** — 가족·연인·친구·신하·선지자·스승제자·대적·군주 등(`스승제자` = 랍비-제자·후계 계승, 엘리↔사무엘·엘리야↔엘리사·예수↔제자). valence가 시간에 따라 변하는 감정 성향이라면, 유형은 그 인물이 상대와 맺는 **고정된 관계 종류**다(pair 단위 1개). 뷰에서 유형별 아이콘으로 표시하고 유형끼리 모아 정렬한다.
...
- **근거 구절 레이어 (evidence verse layer)**: 국면을 누르면 그 관계 근거인 성경 구절 **본문**(한/영)을 레이어로 조회. 구절 본문은 빌드타임 getbible 프리베이크(ADR-0003 선례)로 미리 받아 저장.
